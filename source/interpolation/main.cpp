...
int main() {
    ...
    for (unsigned int i = 1; i < NUM_OF_LENGTHS; ++i) {
        std::array<double, 3> points3 = linspace<double, 3>(0,
        length_array[i]); std::array<double, 3> points3y = exp<double,
        3>(points3); HermiteInterpolator<double, double, 3>
        interpolator3(points3, points3y,
                                                             points3y);
        std::array<double, NUM_OF_DOTS> x =
    ...
    // data.open("data4hermit.txt");
    // std::array<double, 1000> length_array =
    //     powof10<double, 1000>(linspace<double, 1000>(-7, 1));
    // for (unsigned int i = 1; i < 1000; ++i) {
    //     std::array<double, 4> points4 = linspace<double, 4>(0,
    //     length_array[i]); std::array<double, 4> points4y = exp<double,
    //     4>(points4); HermiteInterpolator<double, double, 4>
    //     interpolator4(points4, points4y,
    //                                                          points4y);
    //     std::array<double, 1000> x = linspace<double, 1000>(0,
    ...
    // data.open("data5hermit.txt");
    // std::array<double, 1000> length_array =
    //     powof10<double, 1000>(linspace<double, 1000>(-7, 1));
    // for (unsigned int i = 1; i < 1000; ++i) {
    //     std::array<double, 5> points5 = linspace<double, 5>(0, length_array[i]);
    //     std::array<double, 5> points5y = exp<double, 5>(points5);
    //     HermiteInterpolator<double, double, 5> interpolator5(points5, points5y,
    //                                                          points5y);
