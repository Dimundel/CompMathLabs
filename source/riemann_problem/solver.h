...
double get_max_abs_eigenvalue(const Grid1D<Cell, NX> &grid, const double c) {
    ...
    for (const auto &element : grid.points) {
        double absVelocityPlusC = std::abs(element.velocity + c);
        double absVelocityMinusC = std::abs(element.velocity - c);

        maxAbsEigenvalue = std::max(maxAbsEigenvalue, absVelocityPlusC);
        maxAbsEigenvalue = std::max(maxAbsEigenvalue, absVelocityMinusC);
    }
    return maxAbsEigenvalue;
}

template <unsigned int NX>
void solve_riemann_problem(Grid1D<Cell, NX> grid, const double adiabatic_index,
