...
stats 'data_lw1.txt' nooutput
blocks = STATS_blocks

# Loop through each block and plot it
do for [i=0:blocks-1] {
  plot 'data_lw1.txt' index i using 1:2 title sprintf("Co=1.0"), \
  'data_lw101.txt' index i using 1:2 title sprintf("Co=1.01"), \
  'data_lw06.txt' index i using 1:2 title sprintf("Co=0.6")
}
